...
{% endfor %}';
    ...
    public function coverProductLink($product,$feed){
		...
		$storeUrl= $this->storeManager->getStore($feed->getStoreId())->getBaseUrl();
		
		if(empty($product->getUrlKey())){
			$productUrl= str_replace('.','-', str_replace('--', '-', str_replace(' ', '-', trim(strtolower($product->getName())))));
		}else{
			$productUrl= $product->getUrlKey();
		}
		// \Zend_Debug::dump($productUrl);

		return $productLink= $storeUrl.''.$productUrl.''.$productSuffix;;
    }
    ...
    public function getProductsData($feed)
    {
        ...
        foreach ($productCollection as $product) {
            ...
            // \Zend_Debug::dump(json_decode(json_encode($product->getData())));
            if ($product->getTypeId()=='configurable'){
                ...
                foreach ($child as $value){
                	...
                	// \Zend_Debug::dump($productRepository->get($value->getSku(),false,$feed->getStoreId())->getPrice());
                	if (!isset($finalPrice)) {
                		$finalPrice = $productRepository->get($value->getSku(),false,$feed->getStoreId())->getPrice();
                	}else{
                		if ($productRepository->get($value->getSku(),false,$feed->getStoreId())->getPrice()>$finalPrice) {
                			$finalPrice = $productRepository->get($value->getSku(),false,$feed->getStoreId())->getPrice();
                		}
                	}
                	// test
                	...
                	// \Zend_Debug::dump(get_class_methods($productRepository->get($value->getSku(),false,$feed->getStoreId())));
                }
                // if ($product->getSku() == '0M0007') {
                // 	die('12');
                // }
            ...
            // var_dump($this->storeManager->getStore($feed->getStoreId())
            //         ->getBaseUrl());
            // die();
            ...
            $productLink= $this->coverProductLink($productClone,$feed).$campaignUrl;
            // if ($product->getSku() == '0M0058') {
            //     	die('12');
            //     }
