...
class Captcha extends Template
{
    ...
    {
        $this->getForms();
        $key = in_array($this->_request->getFullActionName(), $this->accessedActionNames);
        if($key && $this->_helperData->isCaptchaFrontend()){
            return true;
        }else {
            return false;
        }
