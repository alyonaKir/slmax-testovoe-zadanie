        ...
        try {
            ...
            $columns = [
                ...
                'total_abandoned_carts'      => new Zend_Db_Expr(
                    'COUNT(CASE WHEN source_table.is_active = 1 AND source_table.updated_at < \'' . date('Y-m-d H:i:s', $currentTimeMinusMeasure) . '\' THEN source_table.is_active END)'
                ),
                ...
                'total_abandoned_revenue'    => new Zend_Db_Expr(
                    'SUM(CASE WHEN source_table.is_active = 1 AND source_table.updated_at < \'' . date('Y-m-d H:i:s', $currentTimeMinusMeasure) . '\' THEN source_table.base_grand_total END)'
                ),
