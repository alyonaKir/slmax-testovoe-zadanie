...
class Attribute extends AbstractValidator implements RowValidatorInterface
{
    ...
    public function checkAttributeOptions($attributesOptions, $multipleSeparator, $optionValueSeparator)
    {
        ...
        if ($attributesOptions != "") {
            ...
            foreach (explode($multipleSeparator, $attributesOptions) as $option) {
                ...
                $checkString = 0 . $optionValueSeparator;
    ...
    /**
     * @param $attributeOptionsSwatch
     * @param $multipleSeparator
     * @return bool
     */
    public function checkAttributeOptionsSwatch($attributeOptionsSwatch, $multipleSeparator)
    {
        $swatchLength = 20;
        if (strlen($attributeOptionsSwatch)>50) {
            $swatchImage = strtolower(substr($attributeOptionsSwatch, 0, 49));
            $swatchLength = $this->setSwatchLength($swatchImage, $swatchLength);
        }
