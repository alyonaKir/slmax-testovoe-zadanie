...
class ResultInterfacePlugin
{
	...
	//start here, added by wayman, convert lite-youtube-embed to AMP version
        $html = preg_replace(
                '<link rel="stylesheet" href="/media/lite-yt-embed.css" />',
                '/br',
                $html);
        
        $VideoIdPos = stripos($html, 'videoid');
        If($VideoIdPos <> NULL)
        {

                $VideoIdStartPos = stripos($html, '"', $VideoIdPos) + 1;
                $VideoIdStopPos = stripos($html, '"', $VideoIdStartPos);
                $VideoId = substr($html, $VideoIdStartPos, $VideoIdStopPos-$VideoIdStartPos);
                $youtubeAmp = '<amp-youtube data-videoid="'.$VideoId.'" layout="responsive" width="480" height="270"></amp-youtube><br />';
                $html = preg_replace(
                     '/<lite-youtube videoid=(.*?)(?:\?|").+?<\/lite-youtube>/s',
                     $youtubeAmp,
                     $html);
        }
        //end here, added by wayman, convert lite-youtube-embed to AMP version
        ...
        //added by wayman to delete the lazy load for images in the product description
